...
type DB struct {
	...
	ops struct {
		...
		doFreedCB    func(fns []base.FileNum)
		checkFreedCB func(fn base.FileNum) bool
	}
...
func Open(path string, opts *options.BdbOptions) (*DB, error) {
	...
	db.ops.doFreedCB = opts.DoFreed
	db.ops.checkFreedCB = opts.CheckFreed
...
func (db *DB) freePagesForTask(pids pgids) {
	...
	var skipPid pgid
	fns := make([]base.FileNum, 0, 16)
	fnsDup := make(map[base.FileNum]struct{})
	...
	parseValue := func(v []byte) {
		...
		fn := base.DecodeFileNum(v)
		if db.ops.checkFreedCB(fn) {
			if _, exist := fnsDup[fn]; !exist {
				fns = append(fns, fn)
				fnsDup[fn] = struct{}{}
			}
		}
