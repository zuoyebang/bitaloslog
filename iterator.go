...
func (i *Iterator) findNextEntry() {
	...
	for i.iterKey != nil {
		...
		case InternalKeyKindSet:
			...
			return
		default:
			i.opts.Logger.Errorf("bitaloslog: Iterator findNextEntry invalid internal key kind %d", key.Kind())
			i.nextUserKey()
			continue
		}
	}
...
func (i *Iterator) findPrevEntry() {
	...
	for i.iterKey != nil {
		...
		case InternalKeyKindSet:
			...
			i.stats.ReverseStepCount[InternalIterCall]++
		default:
			i.err = errors.Errorf("bitaloslog: invalid internal key kind %d", key.Kind())
			i.iterValidityState = IterExhausted
			return
		}
	}
...
func (i *Iterator) SeekPrefixGE(key []byte) bool {
	...
	if lowerBound := i.opts.GetLowerBound(); lowerBound != nil && i.cmp(key, lowerBound) < 0 {
		if n := i.split(lowerBound); !bytes.Equal(i.prefixOrFullSeekKey, lowerBound[:n]) {
			i.err = errors.New("bitaloslog: SeekPrefixGE supplied with key outside of lower bound")
			return false
		}
		key = lowerBound
	} else if upperBound := i.opts.GetUpperBound(); upperBound != nil && i.cmp(key, upperBound) > 0 {
		if n := i.split(upperBound); !bytes.Equal(i.prefixOrFullSeekKey, upperBound[:n]) {
			i.err = errors.New("bitaloslog: SeekPrefixGE supplied with key outside of upper bound")
			return false
		}
		key = upperBound
	}
...
func (i *Iterator) NextWithLimit() IterValidityState {
	...
	case iterPosCurReverse:
		if i.iterKey != nil {
			i.err = errors.New("bitaloslog: switching from reverse to forward but iter is not at prev")
			i.iterValidityState = IterExhausted
			return i.iterValidityState
		}
	...
	case iterPosCurReversePaused:
		if i.iterKey == nil {
			i.err = errors.New("bitaloslog: switching paused from reverse to forward but iter is exhausted")
			i.iterValidityState = IterExhausted
			return i.iterValidityState
		}
		i.nextUserKey()
...
func (i *Iterator) PrevWithLimit() IterValidityState {
	...
	i.lastPositioningOp = unknownLastPositionOp
	if i.hasPrefix {
		i.err = errors.New("bitaloslog: unsupported reverse prefix iteration")
		i.iterValidityState = IterExhausted
		return i.iterValidityState
	}
	switch i.pos {
...
func (i *Iterator) Close() error {
	...
	if i.plainReadState != nil {
		i.plainReadState.unref()
		i.plainReadState = nil
	}
