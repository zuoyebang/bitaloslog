...
const (
	tableItemKeySize    = 2
	tableItemValueSize  = 4
	tableItemHeaderSize = tableItemKeySize + tableItemValueSize
)
...
func (w *tableWriter) set(key InternalKey, value []byte) (uint32, error) {
	keySize := key.Size()
	valueSize := len(value)
	preSize := keySize + tableItemHeaderSize
	wrn := 0
	...
	w.encodeHeader(w.wbuf[:tableItemHeaderSize], uint16(keySize), uint32(valueSize))
	key.Encode(w.wbuf[tableItemHeaderSize:])
