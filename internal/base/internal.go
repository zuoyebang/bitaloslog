...
const (
	InternalValueNormal     = 0
	InternalValueSetBithash = 1
)
...
func DecodeInternalValue(value []byte) (isSeparate bool, v []byte, ev InternalValue) {
	if len(value) == 0 {
		v = []byte("")
		return
	} else if value[0] == InternalValueNormal {
		v = value[1:]
		return
	}
