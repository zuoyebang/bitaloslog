...
import (
	...
	"github.com/zuoyebang/bitaloslog/internal/crc"
)
...
var (
	...
	ErrInvalidChunk = errors.New("bitaloslog/record: invalid chunk")
)
...
func (x singleReader) Read(p []byte) (int, error) {
	r := x.r
	if r.seq != x.seq {
		return 0, errors.New("bitaloslog/record: stale reader")
	}
	if r.err != nil {
		return 0, r.err
	}
...
func (w *Writer) fillHeader(last bool) {
	if w.i+legacyHeaderSize > w.j || w.j > blockSize {
		panic("bitaloslog/record: bad writer state")
	}
	if last {
		if w.first {
...
func (w *Writer) Close() error {
	...
	w.err = errors.New("bitaloslog/record: closed Writer")
	return nil
}
...
func (x singleWriter) Write(p []byte) (int, error) {
	w := x.w
	if w.seq != x.seq {
		return 0, errors.New("bitaloslog/record: stale writer")
	}
	if w.err != nil {
		return 0, w.err
	}
