...
func (y *MemFS) Create(fullname string) (File, error) {
	...
	err := y.walk(fullname, func(dir *memNode, frag string, final bool) error {
		if final {
			if frag == "" {
				return errors.New("bitalosdb/vfs: empty file name")
			}
			n := &memNode{name: frag}
			dir.children[frag] = n
			ret = &memFile{
...
func (*MemFS) GetDiskUsage(string) (DiskUsage, error) {
	return DiskUsage{}, errors.New("bitaloslog: not supported")
}
...
func (f *memFile) Close() error {
	if n := atomic.AddInt32(&f.n.refs, -1); n < 0 {
		panic(fmt.Sprintf("bitaloslog: close of unopened file: %d", n))
	}
	f.n = nil
	return nil
}
...
func (f *memFile) Read(p []byte) (int, error) {
	if !f.read {
		return 0, errors.New("bitalosdb/vfs: file was not opened for reading")
	}
	if f.n.isDir {
		return 0, errors.New("bitalosdb/vfs: cannot read a directory")
	}
	f.n.mu.Lock()
	defer f.n.mu.Unlock()
...
func (f *memFile) Write(p []byte) (int, error) {
	if !f.write {
		return 0, errors.New("bitalosdb/vfs: file was not created for writing")
	}
	if f.n.isDir {
		return 0, errors.New("bitalosdb/vfs: cannot write a directory")
	}
	f.n.mu.Lock()
	defer f.n.mu.Unlock()
